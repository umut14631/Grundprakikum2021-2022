...
theta, Nohne = np.genfromtxt('Messdaten/ComptonOhne.txt', unpack = True)
theta, Nmit = np.genfromtxt('Messdaten/ComptonAl.txt', unpack = True)
...
plt.errorbar(x=wavelength*1e12, y=T, fmt='.', yerr = Terr,  label = 'Transmission', color='red')
plt.plot(x*1e12, params[0] * x + params[1], label = 'Regressionsgerade', color='black')
